...
import com.example.fxy.studentmanage.model.Tb_score;
...
public class ShowScore extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		lvinfo.setOnItemClickListener(new OnItemClickListener(){// 为ListView添加项单击事件
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(ShowScore.this, ScoreInfo.class);// 创建Intent对象
	...
	private void ShowInfo() {// 用来根据传入的管理类型，显示相应的信息
		...
		ScoreDAO scoreDAO = new ScoreDAO(ShowScore.this);// 创建ScoreDAO对象
		// 获取所有收入信息，并存储到List泛型集合中
		List<Tb_score> listinfos = scoreDAO.getScrollCla_StuData(tea_idStr, cla_name,0, (int) scoreDAO.getCla_StuCount(tea_idStr,cla_name,"noaddedview"),"noaddedview");
