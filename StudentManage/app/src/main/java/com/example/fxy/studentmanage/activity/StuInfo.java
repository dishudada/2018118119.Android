package com.example.fxy.studentmanage.activity;
...
import com.example.fxy.studentmanage.R;
import com.example.fxy.studentmanage.dao.ScoreDAO;
import com.example.fxy.studentmanage.dao.StudentDAO;
import com.example.fxy.studentmanage.model.Tb_student;
...
public class StuInfo extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		studentDAO = new StudentDAO(StuInfo.this);
		...
		stu_password.setText(studentDAO.find(stu_idStr).getStu_password());
		...
		btn_save.setOnClickListener(new OnClickListener() {// 为登录按钮设置监听事件
			...
			public void onClick(View v) {
				...
				tb_student.setStu_password(stu_password.getText().toString());// 设置密码

				scoreDAO = new ScoreDAO(StuInfo.this);
				scoreDAO.updateStudent(stu_idStr, stu_name.getText().toString());
				studentDAO.update(tb_student);// 更新学生信息
				
				// 弹出信息提示
				Toast.makeText(StuInfo.this, "数据修改成功！", Toast.LENGTH_SHORT).show();
			}
		});
